...
interface DeepseekKeyValidatorProps {
  apiKey: string;
  onValidationComplete?: (isValid: boolean) => void;
}

export const DeepseekKeyValidator = ({ apiKey, onValidationComplete }: DeepseekKeyValidatorProps) => {
  const [isValidating, setIsValidating] = useState(false);
  const [isValid, setIsValid] = useState<boolean | null>(null);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  const validateDeepseekKey = async () => {
    ...
    try {
      // Try to make a simple models list request to Deepseek API
