...
const MarketOverview = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const isValidData = marketData.every(item => item && typeof item.market === 'string' && typeof item.symbol === 'string' && typeof item.price === 'number');
      if (!isValidData) {
  ...
  const toggleAIInsights = () => {
    ...
    toast({
      ...
      description: showAIInsights ? "Standard market view restored" : "AI-powered market analysis activated",
      duration: 3000
    });
  };
  ...
  if (isInitialLoading) {
    return <div className="w-full h-[200px] flex items-center justify-center bg-secondary/30 backdrop-blur-lg border border-secondary/50 rounded-lg">
  ...
  return <div className="w-full bg-secondary/30 backdrop-blur-lg border border-secondary/50 rounded-lg shadow-lg will-change-transform hover:shadow-xl transition-all duration-300 ease-out">
      <div className="p-6 relative overflow-hidden bg-[#040f04]">
        ...
        <div className="relative flex justify-between items-center mb-6">
          ...
          <Button variant="outline" size="sm" className="flex items-center gap-1.5 hover:bg-primary/20" onClick={toggleAIInsights}>
            <BrainCircuit className="h-4 w-4" />
            {showAIInsights ? "Hide AI Insights" : "Show AI Insights"}
          </Button>
        </div>
        
        {showAIInsights && <div className="mb-6">
            <AIMarketAnalysis marketData={firstMarketData || undefined} className="h-[300px]" />
          </div>}
        ...
        <Tabs defaultValue={marketOrder.find(market => groupedData[market]?.length > 0) || marketOrder[0]} className="w-full">
          <TabsList className="mb-6 bg-background/50 backdrop-blur-md relative flex flex-wrap gap-1">
            ...
            {marketOrder.map(market => groupedData[market]?.length > 0 && <TabsTrigger key={market} value={market} className="relative data-[state=active]:bg-primary/20 data-[state=active]:backdrop-blur-lg transition-all duration-300 ease-out hover:bg-primary/10">
                  {market}
                </TabsTrigger>)}
          </TabsList>

          <div className="h-[500px] transition-transform will-change-transform duration-500 ease-out">
            {marketOrder.map(market => groupedData[market]?.length > 0 && <TabsContent key={market} value={market} className="mt-0 h-full animate-in fade-in-50 duration-500 ease-out">
                  <MarketCharts data={groupedData[market] || []} isLoading={!marketData.length} type="overview" />
                </TabsContent>)}
          </div>
        </Tabs>
      </div>
